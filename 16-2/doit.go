...
func GetFirstGeneration(m *Map) *Generation {
	...
	s := State{valvestatus: 0, actorpos: "AA", elephantpos: "AA"}
	res.states[s] = 0
	res.statetrace[s] = ""
	return &res
}

func (g *Generation) AddState(valvestatus uint64, pos1 string, pos2 string, vnew int) {
	s1 := State{valvestatus: valvestatus, actorpos: pos1, elephantpos: pos2}
	s2 := State{valvestatus: valvestatus, actorpos: pos2, elephantpos: pos1}
	e1, ok1 := g.states[s1]
	e2, ok2 := g.states[s2]
	if ok1 {
		if vnew > e1 {
		...
		if ok2 {
			...
			g.states[s1] = vnew // Avoid creating equivalent state
		}
	}
}
func (g *Generation) GetNextGeneration(m *Map) *Generation {
	...
	for s, v := range g.states {
		// 1. Move to neighbours - both moves
		for p, _ := range m.connections[s.actorpos] {
			for ep, _ := range m.connections[s.elephantpos] {
				vnew := v + s.Income(m)
				res.AddState(s.valvestatus, p, ep, vnew)
			}
		}
		// 2. Open valve - actor, elephant moves
		if !s.IsOpen(s.actorpos, m) && m.valves[s.actorpos] > 0 {
			for ep, _ := range m.connections[s.elephantpos] {
				s1 := State{valvestatus: s.valvestatus, actorpos: s.actorpos, elephantpos: ep}
				vnew := v + s1.Income(m)
				s1.Open(s.actorpos, m)
				res.AddState(s1.valvestatus, s.actorpos, ep, vnew)
			}
		}
		// 3. Open valve - elephant, actor moves
		if !s.IsOpen(s.elephantpos, m) && m.valves[s.elephantpos] > 0 {
			for ap, _ := range m.connections[s.actorpos] {
				s1 := State{valvestatus: s.valvestatus, actorpos: ap, elephantpos: s.elephantpos}
				vnew := v + s1.Income(m)
				s1.Open(s.elephantpos, m)
				res.AddState(s1.valvestatus, ap, s.elephantpos, vnew)
			}
			// Both open valves
			if !s.IsOpen(s.actorpos, m) && m.valves[s.actorpos] > 0 {
				s1 := State{valvestatus: s.valvestatus, actorpos: s.actorpos, elephantpos: s.elephantpos}
				vnew := v + s1.Income(m)
				s1.Open(s.elephantpos, m)
				s1.Open(s.actorpos, m)
				res.AddState(s1.valvestatus, s.actorpos, s.elephantpos, vnew)
			}
		}
